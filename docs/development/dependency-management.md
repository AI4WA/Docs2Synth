...
!!! note "GPU Dependencies"
    GPU PyTorch is installed directly via `pip install torch torchvision torchaudio --extra-index-url https://download.pytorch.org/whl/cu118` rather than from a lockfile, as NVIDIA CUDA dependencies are platform-specific and managed by PyTorch. See `requirements-gpu.in` for documentation.
...
# Note: GPU PyTorch is installed directly from PyTorch index, not from lockfile
```
...
✅ **Reproducible**: Lockfiles ensure identical builds
✅ **Maintainable**: Only edit high-level `.in` files
✅ **Flexible**: Separate scenarios for CPU/GPU/dev
✅ **Standards-based**: Uses PEP 621 `pyproject.toml`
✅ **Compatible**: Works with pip and other tools

### Comparison with Alternatives

| Approach | Speed | Lockfiles | Standards | Complexity |
|----------|-------|-----------|-----------|------------|
| **uv + .in files** | ⚡⚡⚡ | ✅ | ✅ PEP 621 | Low |
| pip + requirements.txt | 🐌 | ❌ | ⚠️ | Low |
| Poetry | ⚡ | ✅ | ⚠️ Custom | Medium |
| Pipenv | ⚡ | ✅ | ⚠️ Custom | Medium |
...
GPU PyTorch requires CUDA 11.8 and is installed directly from the PyTorch index:
...
This approach is used because NVIDIA CUDA dependencies are platform-specific and managed by PyTorch.
...
### GPU Installation on macOS

**Note**: You can install GPU PyTorch on macOS, but it won't have CUDA acceleration (macOS doesn't support NVIDIA CUDA).

```bash
# Install GPU PyTorch (CUDA libraries included but won't be used)
pip install torch torchvision torchaudio --extra-index-url https://download.pytorch.org/whl/cu118

# torch.cuda.is_available() will return False on macOS
```

For actual GPU acceleration, use Linux with NVIDIA GPU.
